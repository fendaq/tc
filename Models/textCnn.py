...
class TextCnn(BasicModel):
    def __init__(self, args, seg_vocab_size, char_vocab_size, seg_matrix, char_matrix):
        ...
        self.SelfAttention = SelfAttention(self.hidden_size, self.hidden_size)
    ...
    def forward(self, fact_seg, accusation_char):
        ...
        # fusions
        accusation_fake_mask = accusation_mask.sum(-1).ne(0).float()
        net = self.Fusion(encoded_fact, encoded_accusation, fact_mask, accusation_fake_mask)
        # self attention
...
#         accusation = self.char_embedding(accusation_char.view(-1, char_len)).view(batch, accusation_num, char_len,
#                                                                                   self.char_embedding_dim)
#         # encodes
#         encoded_fact = self.EncoderFact(fact, fact_mask)
