...
class Fusion(t.nn.Module):
    def __init__(self, fact_dim, accusation_dim, hidden_size):
        ...
        self.fact_accusation_attention = DotAttention(accusation_dim, fact_dim, hidden_size)
        self.fusion_nonlinear = CustomLinear(fact_dim, fact_dim, bias=True, bn=True, act=t.nn.ReLU())
        self.gate = CustomLinear(fact_dim, fact_dim, bias=False, bn=True, act=t.nn.Sigmoid())

    def forward(self, fact, accusation, fact_mask, accusation_mask):
        accusation_info = self.fact_accusation_attention(accusation, fact, accusation_mask, fact_mask)
        fusion_nonlinear = self.fusion_nonlinear(accusation_info)
        gate = self.gate(fusion_nonlinear)
        outputs = gate * fact + (1 - gate) * fusion_nonlinear
        return outputs
